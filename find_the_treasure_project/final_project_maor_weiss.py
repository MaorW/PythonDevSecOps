...
def challenge(steps_to_win):
    ...
    # csvData = pandasForSortingCSV.read_csv('score_file.csv')
    # # csvData.sort_values(by=' Score')
    # new_csv = csvData.sort_values(by=[" Score"], ascending=[False])
    # new_csv.to_csv('score_file.csv')

    return 0  # Lower_score


def get_score_file_parameters():
    score_file = open('score_file.csv')
    reader = csv.reader(score_file)
    num_of_scores = len(list(reader)) - 1  # First row doesn't count as a score
    score_list = list(csv.reader(score_file, delimiter=","))
    print(score_list)
    score_file.close()
...
num_of_scores = get_score_file_parameters()
...
# steps_to_win = 14  # Todo - Remove after debugging
# if steps_to_win >= lower_score or num_of_scores <= 10:
#     lower_score = challenge(steps_to_win)
